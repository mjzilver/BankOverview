...
from analysis import filter_zakelijkheid, summarize_by_counterparty_per_month, summarize_monthly_totals, summarize_monthly_totals_by_label
...
def main():
    ...
    months = summary_df.drop_duplicates("Maand")[["Maand", "Maand_NL"]].sort_values("Maand")
    ...
    st.dataframe(
        filtered_df[["Tegenpartij", "Netto", "Label", "Zakelijk"]].style.format({"Netto": "{:,.2f}"}))

    tab1, tab2, tab3 = st.tabs(["Per Tegenpartij", "Maandelijkse Samenvatting", "Tegenpartij Labels"])
    ...
    with tab1:
        if selected_month != "Alle maanden":
            ...
            st.info("Selecteer een specifieke maand om de grafiek per tegenpartij te zien.")

    with tab2:
        zakelijkheid = st.selectbox(
            "Filter op zakelijkheid",
            ["Alle", "Zakelijk", "Niet-zakelijk"],
            index=0
        )
        

        
        summary_df = filter_zakelijkheid(summary_df, zakelijkheid)
        
        monthly = summarize_monthly_totals_by_label(summary_df)
    ...
    with tab3:
        ...
        for tp in all_parties:
            ...
            if (
                (not existing.empty and (label != existing["Label"].values[0] or (zakelijk == "Zakelijk") != existing["Zakelijk"].values[0]))
                or (existing.empty and (label or zakelijk))
            ):
